...
class LinksController extends FrontController
{
    ...
    public function beforeFilter(Event $event)
    {
        ...
        $this->Auth->allow(['shorten', 'view', 'go', 'popad', 'page2nd', 'page3rd', 'page4th', 'page5th']);
    ...
    {
        $id = $this->request->getParam('alias');

        $table = $this->loadModel('ads');
        $data = $this->ads->find()->where(['id' => 1])->first();
    ...
    public function page5th()
    {
        $id = $this->request->getParam('alias');
        $this->loadModel('links');
        $link = $this->links->find()->where(['alias' => $id])->first();
        $users = $link['url'];
